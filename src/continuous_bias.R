...
outputfile <- "bias_results_comparison_different_alpha.csv"
...
beta12 <- -0.7*c(log(1),log(3),log(4),log(3),log(2))
alpha1 <- -1.9 + 0.6*log(times)
alpha2 <- -2.5 + 0.6*log(times)
...
sampling.discrete.competing.Cox <- function(i) {
  ...
  cen.prob <- c(rep(0.005,max(times)),1-sum(rep(0.005,max(times))))
...
for (sam in 1:rep) {
  ...
  while (minimal_occurrences < 5) {
    ...
    # Filter the data for J == 1 or J == 2
    filtered_df <- subset(dat, statusJ %in% c(1, 2))
    
    # Count observations grouped by X and J, and get the minimum count
    minimal_occurrences <- min(table(filtered_df$time, filtered_df$statusJ))
  }
  ...
  print('Ours')
  fit1.short <- clogit(delta1 ~ cov1+cov2+cov3+cov4+cov5  + strata(time), data = Expdata, method = "exact") # , method = "exact"
  fit2.short <- clogit(delta2 ~ cov1+cov2+cov3+cov4+cov5  + strata(time), data = Expdata, method = "exact") # , method = "exact"
