...
beta11 <- -c(log(0.8),log(3),log(3),log(2.5),log(2))
beta12 <- -c(log(1),log(3),log(4),log(3),log(2))
alpha1 <- -2 - 0.2*log(times)
alpha2 <- -2.2 - 0.2*log(times)
...
sampling.discrete.competing.Cox <- function(i) {
  ...
  cen.prob <- c(rep(0.05,max(times)),1-sum(rep(0.05,max(times))))
...
for (sam in 1:rep) {
  ...
  fit1.short <- clogit(delta1 ~ cov1+cov2+cov3+cov4+cov5  + strata(time), method = "approximate", data = Expdata)
  fit2.short <- clogit(delta2 ~ cov1+cov2+cov3+cov4+cov5  + strata(time), method = "approximate", data = Expdata)
...
results.dat <- as.data.frame( cbind(true.coef,est.coef,est.coef.fast,empirical.se,est.se,empirical.se.fast) )
